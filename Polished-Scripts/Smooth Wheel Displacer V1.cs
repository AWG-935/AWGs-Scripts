 ...
 * -----------
 * V1
 * Modified variant of the smooth rotor displacement script, but for wheels!
 * Slightly friendlier, slightly less accurate in displacement (a few cm off)
 * but you can get it close. Just put your wanted position in an argument and run.
 * 
 */
...
float StoppingPrecision = 8f; //What the ErrorMargin distance is divided by when calculating how precisely to stop. (Higher = more precise)
...
const string Name_Group_DisplacementWheels =        //Light group that will change colour based on system status
"Displacement Wheels";
...
//---------------------//
//NO TOUCH BELOW
List<IMyMotorSuspension> List_DisplacementWheels = new List<IMyMotorSuspension>();
...
public void Main(string argument, UpdateType updateSource)
{
    ...
    if (argument != "")
    {
        ...
        if (float.Parse(argument) > Math.Round(List_DisplacementWheels[0].Height - (DisplacementErrorMarginDistance / 2), 4))
        {
    ...
    if (HasWheelGroup == false)
    {
        Echo("NO GROUP");
        ErrorThrownThisRun = true;
        Function_CompileDisplayedErrorMessage(ref ErrorLog, Name_Group_DisplacementWheels);
        return;
    }
    else if (DisplacementTargetSatisfied == false) //basically have you just switched something around
    {
        Function_DisplaceWheels(List_DisplacementWheels, DisplacementTarget, ref ScreenPrint);
...
{
    IMyBlockGroup Group_DisplacementWheels = GridTerminalSystem.GetBlockGroupWithName(Name_Group_DisplacementWheels); //Self explanatory

    if (Group_DisplacementWheels != null)
    {
        Echo("Wheel Group found");
        HasWheelGroup = true;
        Group_DisplacementWheels.GetBlocksOfType(List_DisplacementWheels);//(This is just here to make sure you're working with the rotors in the group, looks in the group for the block type from the list specified)
    }
    else
    {
        Echo("No Wheel Group named \n"+ Name_Group_DisplacementWheels + " found");
        HasWheelGroup = false;
        return false;
    }
...
public void Function_DisplaceWheels(List<IMyMotorSuspension> List_DisplacementWheels, float WheelDisplacement, ref string ScreenPrint)//Instead of repeating a bulky segment twice, I pass a colour into this, and run it once depending on what's going
{
    if (Me.CustomData != Message_ActionComplete)
    {
        if (HasWheelGroup)
        {
            ...
            foreach (var Group_DisplacementWheels in List_DisplacementWheels)//for every rotor in the DisplacementRotors list made from the DisplacementRotors group, recognised as a MotorStator
            {
                if (Group_DisplacementWheels.Height < (DisplacementTarget - DisplacementErrorMarginDistance) || Group_DisplacementWheels.Height > (DisplacementTarget + DisplacementErrorMarginDistance))
                {
                    Group_DisplacementWheels.Height = Group_DisplacementWheels.Height + Displacement_Speed;
                    Echo("High speed");
                ...
                {
                    Group_DisplacementWheels.Height = Group_DisplacementWheels.Height + (Displacement_Speed / Displacement_SlowDownDivision);
                    Echo("Slow speed");
                }

                if (Math.Round(Group_DisplacementWheels.Height, 4) < DisplacementTarget + (DisplacementErrorMarginDistance / StoppingPrecision) && Math.Round(Group_DisplacementWheels.Height, 3) > DisplacementTarget - (DisplacementErrorMarginDistance / StoppingPrecision))
                {
