 ...
 * 
 * Quick changelog: Added left/right timer support and A/D recognition. This may be buggy, but should work well enough. 
 * Now automatically configures your text panel too - this is customisable as ever.
 */

const string Name_ControllerTag = "Driver"; //Driver Seat (currently only one seat at once)

const string StatusPanel = "Transmission Panel"; //Gear display Panel

float MaximumSpeed = 60f; //The top limit of your vehicle's speed, used as a clamp for all others'
const float ReverseSpeed = 60f; //Your top speed when reversing
const float ForwardGearCount = 6f; //The divisions of your MaximumSpeed that make up your forward gears. (100km/h top speed with 4 gears, means gear 1 is 25km/h, 2 is 50km/h, so on)
const float ReverseGearCount = 6f; //The same as above, but for reverse

const string ForwardGearDefault = "4"; //What gear does accelerating put you in by default? (Don't go above max)
...
bool ADControl = false; //Whether to send signals out on A/D usage. This controls specific timers and can be used for animated track or rotor control.
...
string TimerLeft = "TimerLeft";
string TimerRight = "TimerRight";
...
bool UsingLeftTimer = true;
bool UsingRightTimer = true;
//The left/right inputs will trigger the neutral timer when you stop using them!

//Status Panel Customisation//
//-------------------------//

const string Title = "Transmission"
    + "\n" + "------------" + "\n";//This is the linebreak for the title. Remove if wanted but keep the semicolon.
const string GearName = Title + "Gear"; //output to the status panel
...
const float Panel_FontSize = 2.130f;

Color Colour_Foreground = new Color
(255, 92, 0);                                              //What it switches to when storage is empty

Color Colour_Background = new Color
(24, 0, 0);

const string Name_ProgramTag2 = "#TR";//Tag for my other scripts to find this one and grab wheel speed.
...
IMyTimerBlock TimerLeftBlock;
IMyTimerBlock TimerRightBlock;
...
public void Main(string argument, UpdateType updateType)
{
    ...
    if (TimerUse == true)
    {
        ...
        TimerLeftBlock = GridTerminalSystem.GetBlockWithName(TimerLeft) as IMyTimerBlock;
        TimerRightBlock = GridTerminalSystem.GetBlockWithName(TimerRight) as IMyTimerBlock;
    }
    ...
    if (TimerUse == true)
        if (TimerShutoff == true)
        {
            ...
            if (UsingLeftTimer == true)
            {
                if (TimerLeftBlock == null)
                {
                    TimerUse = false;
                }
            }

            if (UsingRightTimer == true)
            {
                if (TimerRightBlock == null)
                {
                    TimerUse = false;
                }
            }
    ...
    try
    {
        ...
        if (inputVec.Z < 0) //W if input is positive in the forwards direction
        {
            ...
            Function_TimerTrigger(TimerForwardBlock, TimerUse, true);
        ...
        else if (inputVec.Z > 0)//S
        {
            ...
            Function_TimerTrigger(TimerReverseBlock, TimerUse, true);
        }
        ...
        if (inputVec.Z == 0) //Nor forwards or backwards
        {
            ...
            {
                if (TimerNeutralBlock != null && TimerNeutralBlock.ShowInInventory == true)//if you can find the block for neutral timer
                {

                    if (TimerForwardBlock != null)//if the forward block is there
                    {
                        if (TimerForwardBlock.ShowInInventory == false)//if the reverse block has run
                        {
                    ...
                    if (TimerReverseBlock != null) // if it exists
                    {
                        if (TimerReverseBlock.ShowInInventory == false)//if the reverse block has run
                        {
        ...
        if (ADControl == true)
        {
            ...
            if (inputVec.X < 0) //A (LEFT)
            {
                Echo("Left");
                TimerNeutralBlock.Enabled = false;
                if (TimerUse == true && TimerLeftBlock != null && TimerLeftBlock.ShowInInventory == true)
                {
                    TimerLeftBlock.Trigger();
                    TimerLeftBlock.ShowInInventory = false;
                }
            }
            else if (inputVec.X > 0) //D (RIGHT)
            {
                Echo("Right");
                TimerNeutralBlock.Enabled = false;
                if (TimerUse == true && TimerRightBlock != null && TimerRightBlock.ShowInInventory == true)
                {
                    TimerRightBlock.Trigger();
                    TimerRightBlock.ShowInInventory = false;
                }
            }


            if (inputVec.X == 0 && TimerLeftBlock.ShowInInventory == false || inputVec.X == 0 && TimerRightBlock.ShowInInventory == false)
            {
                TimerNeutralBlock.Enabled = true;
                TimerLeftBlock.ShowInInventory = true;
                TimerRightBlock.ShowInInventory = true;
                if (inputVec.Z == 0)
                {
                    TimerNeutralBlock.Trigger();
                }
                else if (inputVec.Z < 0)//W
                {
                    Function_TimerTrigger(TimerForwardBlock, TimerUse, false);
                }
                else if (inputVec.Z > 0)//S
                {
                    Function_TimerTrigger(TimerReverseBlock, TimerUse, false);
                }
            ...
            if (inputVec.X != 0)
            {
                ...
                {
                    if (TimerReverseBlock != null)//if the forward block is there
                    {
                        if (TimerReverseBlock.ShowInInventory == false)//if the reverse block has run
                        {
                            TimerReverseBlock.ShowInInventory = true;
    ...
    else
    {
        ...
        {
            if (echopanel.ContentType == ContentType.NONE)
            {
                echopanel.ContentType = ContentType.TEXT_AND_IMAGE;
                echopanel.Font = "Monospace";
                echopanel.FontSize = Panel_FontSize;
                echopanel.FontColor = Colour_Foreground;
                echopanel.BackgroundColor = Colour_Background;
                echopanel.Alignment = TextAlignment.CENTER;
            }
...
Count_RunTimer += 1;
    Echo("\nController Refresh: " + (Count_RunTimer* 2f + "%\n"));

    if (Count_RunTimer == 50)
    {
        Count_RunTimer = 1;


        if (!Me.CustomName.Contains(Name_ProgramTag2))//if the program doesn't have Name_ControllerTag, add it
        {
            Me.CustomName += (" " + Name_ProgramTag2);
        }
    }
...
public void Function_TimerTrigger(IMyTimerBlock TimerBlock, bool TimerUse, bool ResetShowInventoryTrigger)
{

    if (TimerUse == true)//Forwards timer systems
    {
        if (TimerBlock != null)//if you can find the block for forward timer
        {
            if (TimerBlock.ShowInInventory == true)
            {
                TimerBlock.ShowInInventory = !ResetShowInventoryTrigger;
                TimerBlock.Trigger();
