/*
 * AWG's Electrical Transmission Script
 * -------------------------------------
 ...
 * one is recommended. - Partial remote control support - you need to name the remote the same as 'DriverSeatName'
 * and cannot have a seat named the same. It can't distinguish between blocks that are controlled and named the same.
 */

const string DriverSeatName = "Driver Seat"; //Driver Seat (currently only one seat at once)

const string StatusPanel = "Status Panel"; //Gear display Panel

const float MaximumSpeed = 80f; //The top limit of your vehicle's speed, used as a clamp for all others'
